...
(function($) {
	$.fn.mChange = function(options) {
		...
		// 		cnow=c.eq(idx);
		// 		cnow.stop().transition({opacity:1,scale:1,zIndex:1},speed*0.8,'cubic-bezier(0,1,1,1)',function(){
		...
		function change(idx){
			if(c.size()>1 || idx=="begin"){
				...
				if(idx=="begin"){
					...
					speed=opts.speed;
					cnow.stop().transition({opacity:0,scale:0.5},speed,'cubic-bezier(0.5,0,0.2,1)',function(){
						$(this).css({scale:1,zIndex:-1});
					});
				}
				...
				cnow.stop().transition({opacity:1,scale:1,zIndex:1},speed*0.8,'cubic-bezier(0,1,1,1)',function(){
					...
					$(this).transition({scale:1},0000,'linear');
					sw=1;
				});
				opts.animates(idx);//执行自定义动画

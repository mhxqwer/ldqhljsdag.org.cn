...
$(function(){
	...
	$('.pop-text .close').on('click', function() {
		$('.pop-text').fadeOut(300);
	});

	$('.share-box a.share-item').on('click',function() {
...
//	sideBarPos('.side-bar',1250,45);			//	侧边栏定位调用
	...
	function getSubnavPos(){
		$('.nav-lv1 > .nav-item').each(function(){
			...
			left = (twidth + posl - $(window).width() / 2) *2;
			$this.find('.nav-grp').css({'margin-left': left + 'px'});
		});
	}
...
/* ******************************************************************************************************************* */
function disMove(ele){
	remove(ele)
	$(ele).find('.list').on("afterChange",function(e, s, c){
		remove(ele)
	});
}

function remove(ele){
	if($(ele).find(".slick-prev").size() == 0){
		$(ele).find(".fun").hide();
	}
	if($(ele).find(".slick-prev").hasClass("slick-disabled")){
		$(ele).find(".fun .prev").addClass("disabled");
	}else{
		$(ele).find(".fun .prev").removeClass("disabled");
	};
	if($(ele).find(".slick-next").hasClass("slick-disabled")){
		$(ele).find(".fun .next").addClass("disabled");
	}else{
		$(ele).find(".fun .next").removeClass("disabled");
	};

}

function disMove1(ele){
	remove1(ele)
	$(ele).find('.slick-mod').on("afterChange",function(e, s, c){
		remove1(ele)
	});
}

function remove1(ele){
	if($(ele).find(".slick-prev").size() == 0){
		$(ele).find(".fun").hide();
	}
	if($(ele).find(".slick-prev").hasClass("slick-disabled")){
